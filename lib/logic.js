...
async function issue(transaction) {
    ...
    console.log("TO HASH")
    console.log(JSON.stringify(serializer.toJSON(certificate, {      validate: false,       convertResourcesToRelationships: true,      permitResourcesForRelationships: false,      deduplicateResources: false   })));
    var hashed = CryptoJS.SHA256(JSON.stringify(serializer.toJSON(certificate, {
        validate: false, 
        convertResourcesToRelationships: true,
        permitResourcesForRelationships: false,
        deduplicateResources: false
     }))); 
    var result = hashed.toString()
    abstraction.certHash = result;
    // console.log(JSON.stringify(serializer.toJSON(certificate, {validate: false,     convertResourcesToRelationships: true,      permitResourcesForRelationships: false,      deduplicateResources: false  })))
